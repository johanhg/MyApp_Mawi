class DatosController < ApplicationController
  ...
  respond_to do |format|
    if @dato.amount > 100 and @dato.amount < 2000
      if @dato.save
        format.html { redirect_to @dato, notice: 'Datum was successfully created.' }
      ...
      @dato = Dato.new
      format.html { render :new, notice: 'El amount debe de ser mayor a 100 y menor a 2000' }
      format.json { render json: @dato.errors, status: :unprocessable_entity }
    end
  end
end
  ...
  private
    ...
    def dato_params
      params.require(:dato).permit(:name, :email, :totalIngress, :sector, :workYears, :amount, :frecuency, :payTime)
    end
end
